...
export async function loadRecipes(allVeg) {
    // const vegName = ['apple', 'lettuce']
    // const vegArray = []
    const myVeg = allVeg[0]
    // for (const veg of myVeg) {
    const res = await fetch(
        `https://api.edamam.com/api/recipes/v2?type=public&q=${myVeg}&app_id=79e5508c&app_key=${APIKey}&dishType=Main%20course&imageSize=THUMBNAIL&field=label&field=image&field=url&field=ingredientLines&field=totalCO2Emissions`
    )
    const recipeArray = await res.json()
    // vegArray.push(recipeArray)
    return { fetched: recipeArray }
}
